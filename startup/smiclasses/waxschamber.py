...
class Sample_Chamber(Device):
    ...
    svent_valve = Cpt(TwoButtonShutter, 'XF:12IDC-VA:2{Det:300KW-VVSoft:WAXS}')
    fvent_valve = Cpt(TwoButtonShutter, 'XF:12IDC-VA:2{Det:300KW-VV:WAXS}')
    air_valve = Cpt(TwoButtonShutter, 'XF:12IDC-PU{Air-Vlv:Supply}')
    N2_valve = Cpt(TwoButtonShutter, 'XF:12IDC-PU{N2-Vlv:Supply}')
    turbo_valve = Cpt(TwoButtonShutter, 'XF:12IDC-PU{Air-Vlv:Supply}')
    turbo_cooling_valve = Cpt(TwoButtonShutter, 'XF:12IDC-PU{PCHW-Vlv:Supply}')
    ...
    vent_safe = Cpt(EpicsSignal, 'XF:12ID2-ES{AutoBleed}Plus-Cmd',put_complete=False)
    ...
    def pump(self):
        yield from bps.mv(
            self.svent_valve,'Retract',
            self.fvent_valve,'Retract',
            self.air_valve,'Retract',
            self.N2_valve,'Retract',timeout=2)
        yield from bps.sleep(1)
        yield from bps.mv(
            self.turbo_valve,'Insert',timeout=2)
        yield from bps.sleep(.5)
        yield from bps.mv(
            self.turbo_cooling_valve,'Insert',timeout=2)
        yield from bps.sleep(.5)
        yield from bps.mv(
            self.turbo_enable,1,
