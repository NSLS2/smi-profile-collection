...
from smi_beamline.devices.electrometers import (
    Keithly2450,
    PinDiodeTetrAMM,
    XBPM,
    new_LakeShore,
...
# BPM2/BPM3 sums are RECORDED (kind='normal') but NOT auto-plotted. They land in the
# baseline and, when read into a scan's primary stream, would otherwise be hinted and clutter every
# plot. To plot a flux channel deliberately, set e.g. ``xbpm3.sumY.kind = 'hinted'`` for a session.
...
# Experimental alternate view of the same TetrAMM. This does not replace the existing
# ``pin_diode`` object; use it to test useful readout/configuration PVs.
